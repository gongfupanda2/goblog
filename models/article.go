...
func GetArticle(id int) (Article, error) {
	...
	if err != nil {
		...
		art = Article{Id: id}
		err = o.Read(&art, "id")
		buf:=strings.Replace(art.Content, "???", "", -1)
		art.Content=buf
...
//
func ListBySearch(keyword string , page int, numPerPage int) ([]orm.Params, bool, int, error) {
	sql1 := "select *,replace(content,'???','') as content1 ,date_format(time,'%Y') as year,date_format(time,'%m') as month from article where match(title,content) against (? in natural language mode)order by time desc limit ?,?"
	sql2 := "select count(*) as number from article where match(title,content) against (? in natural language mode)"
	...
	var i int64
	for i = 0; i < num; i++ {
		maps[i]["content1"]=strings.Replace(maps[i]["content1"].(string),keyword,"<font color=\"FF0000\">"+keyword+"</font>",-1)
	}
...
func ListByMonth(year int, month int, page int, numPerPage int) ([]orm.Params, bool, int, error) {
	...
	if nil != err {
		sql1 := "select *,replace(content,'???','') as content1 from article where year(time)=? and month(time)=? order by time desc limit ?,?"
...
func ListPage(page int, numPerPage int) ([]orm.Params, bool, int, error) {
	// pagePerNum := 6
	sql1 := "select *,replace(content,'???','') as content1,date_format(time,'%Y') as year,date_format(time,'%m') as month from article order by time desc limit ?," + fmt.Sprintf("%d", numPerPage)
...
func ListByKeyword(keyword string, page int, numPerPage int) ([]orm.Params, bool, int, error) {
	// numPerPage := 6
	sql1 := "select *,replace(content,'???','') as content1 ,date_format(time,'%Y') as year,date_format(time,'%m') as month from article where keywords like ? order by time desc limit ?,?"
