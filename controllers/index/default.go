...
func (this *SearchController) Get() {
	tag := this.Ctx.Input.Param(":keywords")
	...
	maps, nextPageFlag, totalPages, err := ListBySearch(tag, int(page), 6)
	...
	if nil == err {
		this.Data["prev_page"] = fmt.Sprintf("/search/%s/%d", tag, page-1)
		this.Data["prev_page_flag"] = prevPageFlag
		this.Data["next_page"] = fmt.Sprintf("/search/%s/%d", tag, page+1)
		this.Data["next_page_flag"] = nextPageFlag
		this.Data["articles_in_page"] = maps
	}
